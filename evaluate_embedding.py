...
def parallel_cal(x, y, split_):
    # split_, x, y = arg
    search=True
    train_index, test_index = split_
    # test
    ...
    acc_val = accuracy_score(y_test, classifier.predict(x_test))
    return acc_test,acc_val

def svc_classify_parallel(x, y, search):
    kf = StratifiedKFold(n_splits=10, shuffle=True, random_state=None)
    # pdb.set_trace()
    # accuracies = []
    # accuracies_val = []

    split_list = list(kf.split(x, y))

    pool = multiprocessing.Pool(10)
    # accuracies, accuracies_val = zip(*pool.map(parallel_cal, iterable=[*zip(split_list, [x for i in range(10)], [y for i in range(10)])]))
    accuracies, accuracies_val = zip(*pool.map(partial(parallel_cal, x, y), split_list))
...
def svc_classify_test(x, y, search):
    kf = StratifiedKFold(n_splits=10, shuffle=True, random_state=None)
    accuracies = []
    # accuracies_val = []
    for train_index, test_index in kf.split(x, y):
...
def evaluate_embedding(embeddings, labels, search=True):
    ...
    _acc_val, _acc = svc_classify_parallel(x,y, search)
    if _acc_val > acc_val:
        acc_val = _acc_val
        acc = _acc
